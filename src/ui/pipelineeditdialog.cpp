...
void PipelineEditDialog::setAutoGeneratedCode(int id, const QString &type)
{
    // 根据类型生成前缀
    QString prefix;
    if (type == "water_supply") {
        prefix = "WS";  // Water Supply
    } else if (type == "sewage") {
        prefix = "SW";  // SeWage
    } else if (type == "gas") {
        prefix = "GS";  // GaS
    ...
    // 生成编号：前缀-三位ID，例如 WS-001
    QString code = QString("%1-%2").arg(prefix).arg(id, 3, 10, QChar('0'));
    m_codeEdit->setText(code);
    
    qDebug() << "✨ Auto-generated pipeline code:" << code;
}
