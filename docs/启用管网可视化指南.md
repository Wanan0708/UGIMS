# 🗺️ 启用管网可视化完整指南

**日期**: 2024-11-27  
**状态**: ✅ 已启用管网可视化代码  
**前提**: PostgreSQL环境正常

---

## 📋 快速开始

### 方式A: 自动测试（推荐）⭐⭐⭐⭐⭐

```bash
# 1. 测试数据库环境
.\scripts\test_database.bat

# 2. 如果测试通过，启动程序
.\release\UGIMS.exe
```

### 方式B: 手动步骤

按照下面的详细步骤逐步完成。

---

## 🔍 详细步骤

### 步骤1: 检查PostgreSQL服务

#### Windows

```bash
# 查看服务状态
sc query | findstr postgresql

# 或使用服务管理器
services.msc
# 查找 postgresql-x64-16 或类似名称

# 启动服务（如果未运行）
net start postgresql-x64-16
```

**预期结果**:
```
SERVICE_NAME: postgresql-x64-16
STATE      : 4  RUNNING
```

---

### 步骤2: 检查数据库配置

#### 检查配置文件

```bash
# 查看配置
type config\database.ini
```

**关键配置**:
```ini
[database]
type=postgresql
host=localhost
port=5432
dbname=ugims
username=postgres
password=你的密码  # ⚠️ 修改这里！
```

#### 修改密码

```bash
# 编辑配置文件
notepad config\database.ini

# 将password=your_password改为你的实际密码
password=你的PostgreSQL密码
```

---

### 步骤3: 创建数据库和导入数据

#### 方式A: 使用测试脚本（推荐）

```bash
.\scripts\test_database.bat
```

**脚本会自动**:
1. ✅ 检查PostgreSQL服务
2. ✅ 测试数据库连接
3. ✅ 创建ugims数据库（如不存在）
4. ✅ 启用PostGIS扩展
5. ✅ 导入数据库结构
6. ✅ 导入测试数据

#### 方式B: 手动执行

```bash
# 1. 连接PostgreSQL
psql -U postgres

# 2. 创建数据库
CREATE DATABASE ugims ENCODING 'UTF8';

# 3. 连接到新数据库
\c ugims

# 4. 启用PostGIS
CREATE EXTENSION IF NOT EXISTS postgis;
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

# 5. 退出psql
\q

# 6. 导入数据库结构
psql -U postgres -d ugims -f database\schema.sql

# 7. 导入测试数据
psql -U postgres -d ugims -f database\test_data.sql

# 8. 验证数据
psql -U postgres -d ugims -c "SELECT COUNT(*) FROM pipelines;"
psql -U postgres -d ugims -c "SELECT COUNT(*) FROM facilities;"
```

**预期输出**:
```
管线数量: 5
设施数量: 4
```

---

### 步骤4: 测试数据库连接（可选）

#### 使用psql测试

```bash
# 测试连接
psql -U postgres -d ugims -c "SELECT version();"

# 测试PostGIS
psql -U postgres -d ugims -c "SELECT PostGIS_Version();"

# 查看数据
psql -U postgres -d ugims -c "SELECT pipeline_type, COUNT(*) FROM pipelines GROUP BY pipeline_type;"
```

#### 使用C++测试程序

```bash
# 编译测试程序
.\scripts\compile_test_db.bat

# 运行测试
.\test_db_connection.exe
```

**预期输出**:
```
========================================
数据库连接测试
========================================

[1/4] 加载配置文件...
✅ app.ini加载成功
✅ database.ini加载成功

[2/4] 初始化数据库管理器...
✅ 数据库管理器初始化成功

[3/4] 连接数据库...
数据库类型: postgresql
主机: localhost
端口: 5432
数据库名: ugims
用户名: postgres
✅ 数据库连接成功！

[4/4] 测试数据查询...

=== 管线统计 ===
给水: 2条
排水: 1条
燃气: 1条
电力: 1条

=== 设施统计 ===
阀门: 2个
检查井: 2个

✅ 数据查询成功！

========================================
数据库连接测试完成 ✅
========================================
```

---

### 步骤5: 启动程序

```bash
# 启动程序
.\release\UGIMS.exe

# 或使用详细输出
.\scripts\test_startup_verbose.bat
```

---

## 📊 程序启动流程

### 时间线

```
0ms    - 程序启动
         ├─ setupUi()
         └─ setupFunctionalArea()
         
100ms  - 窗口显示 ✅
         └─ setupMapArea() 开始
         
500ms  - 地图初始化完成 ✅
         
800ms  - 管网可视化初始化开始
         ├─ Logger初始化
         ├─ Config加载
         ├─ 数据库配置加载
         └─ 数据库连接 (timeout: 5s)
         
3s     - 数据库连接完成 ✅
         ├─ LayerManager创建
         ├─ SymbolManager初始化
         ├─ 渲染器创建
         └─ 加载管网数据
         
5s     - 管网数据渲染完成 ✅
```

### 关键日志

**成功的日志**:
```
[INFO] MyForm constructor finished (UI setup only)
[INFO] Starting async map area initialization
[INFO] TileMapManager created
[INFO] Map area initialization complete
[INFO] Starting async pipeline visualization initialization
[Pipeline] Step 1: Initialize Logger
[Pipeline] ✅ Logger initialized
[Pipeline] Step 2: Load Config
[Pipeline] ✅ App config loaded
[Pipeline] Step 3: Load Database Config
[Pipeline] ✅ Database config loaded
[Pipeline] Step 4: Connect to Database
[Pipeline] Connecting to database (timeout: 5s)...
[Pipeline] ✅ Database connected
[INFO] PostGIS version: 3.4.0
[Pipeline] Step 5: Create LayerManager
[Pipeline] ✅ LayerManager created
[Pipeline] ✅ Initialization complete
```

**失败的日志** (数据库未连接):
```
[Pipeline] Step 3: Load Database Config
[Pipeline] ⚠️  No database config - visualization disabled
[INFO] 数据库配置未找到（地图功能正常）
```

---

## 🎯 预期效果

### 成功启动后

✅ **窗口快速显示** (< 0.5秒)  
✅ **蓝色渐变主题** 正确加载  
✅ **地图正常显示** OpenStreetMap瓦片  
✅ **状态栏提示** "数据库已连接，准备加载管网数据..."  
✅ **管线显示** 5-10秒后在地图上显示彩色管线  
✅ **设施显示** 阀门、检查井等设施点

### 地图上的显示

**管线颜色**:
- 🔵 **蓝色** - 给水管线
- 🟤 **棕色** - 排水管线
- 🟡 **金色** - 燃气管线
- 🔴 **红色** - 电力管线
- 🟢 **绿色** - 通信管线

**设施图标**:
- 🟢 **绿色圆点** - 阀门
- 🔵 **蓝色圆点** - 检查井
- 🟠 **橙色圆点** - 其他设施

### 测试数据位置

默认测试数据位于**北京天安门附近**:
- 中心点: 39.9042°N, 116.4074°E
- 缩放等级: 15-17
- 范围: 约500米半径

---

## ⚠️ 故障排除

### 问题1: 程序卡死

**症状**: 窗口显示后无响应

**解决方案**:
1. 检查PostgreSQL服务是否运行
2. 检查密码是否正确
3. 检查防火墙是否阻止
4. 查看日志 `logs\app.log`

```bash
# 查看最后50行日志
Get-Content logs\app.log -Tail 50
```

---

### 问题2: 数据库连接超时

**症状**: 
```
[Pipeline] ❌ DB connect failed: timeout
```

**解决方案**:
```bash
# 1. 检查服务
sc query | findstr postgresql

# 2. 启动服务
net start postgresql-x64-16

# 3. 测试连接
psql -U postgres -c "SELECT 1"
```

---

### 问题3: 密码错误

**症状**:
```
[Pipeline] ❌ DB connect failed: 数据库密码错误
```

**解决方案**:
```bash
# 1. 修改配置文件
notepad config\database.ini

# 2. 或重置PostgreSQL密码
psql -U postgres
ALTER USER postgres WITH PASSWORD '新密码';
```

---

### 问题4: 数据库不存在

**症状**:
```
[Pipeline] ❌ DB connect failed: 数据库不存在
```

**解决方案**:
```bash
# 运行数据库测试脚本，会自动创建
.\scripts\test_database.bat

# 或手动创建
createdb -U postgres ugims
psql -U postgres -d ugims -f database\schema.sql
psql -U postgres -d ugims -f database\test_data.sql
```

---

### 问题5: 地图上看不到管网

**可能原因**:
1. 缩放等级不合适（太远或太近）
2. 中心点不在数据范围内
3. 图层被隐藏
4. 数据渲染失败

**解决方案**:
1. 调整缩放等级到15-17
2. 移动地图到北京天安门附近 (116.4074, 39.9042)
3. 检查日志中的渲染错误
4. 查看数据库中是否有数据

```sql
-- 检查数据
psql -U postgres -d ugims
SELECT id, pipeline_type, ST_AsText(geometry) FROM pipelines LIMIT 1;
```

---

## 📝 检查清单

### 启动前检查

- [ ] PostgreSQL服务运行中
- [ ] config/database.ini密码正确
- [ ] ugims数据库已创建
- [ ] PostGIS扩展已启用
- [ ] 数据表已导入
- [ ] 测试数据已导入

### 启动后检查

- [ ] 窗口正常显示
- [ ] 地图可以拖拽缩放
- [ ] 状态栏显示"数据库已连接"
- [ ] 日志中无错误
- [ ] 地图上显示管线（5-10秒后）
- [ ] 可以点击查看管线属性

---

## 🔗 相关命令

### PostgreSQL常用命令

```bash
# 连接数据库
psql -U postgres -d ugims

# 列出所有数据库
\l

# 列出所有表
\dt

# 查看表结构
\d pipelines

# 退出
\q

# 直接执行SQL
psql -U postgres -d ugims -c "SELECT COUNT(*) FROM pipelines;"

# 导出数据
pg_dump -U postgres ugims > backup.sql

# 导入数据
psql -U postgres -d ugims -f backup.sql
```

---

## 🎉 成功标志

当你看到以下内容时，说明一切正常：

✅ 程序快速启动  
✅ 地图正常显示  
✅ 状态栏: "数据库已连接，准备加载管网数据..."  
✅ 日志: `[Pipeline] ✅ Database connected`  
✅ 日志: `[INFO] PostGIS version: 3.x.x`  
✅ 地图上出现彩色管线  
✅ 可以缩放和拖拽  

**恭喜！管网可视化功能已成功启用！** 🎉🗺️

---

## 📚 相关文档

- [DATABASE_SETUP.md](DATABASE_SETUP.md) - 详细数据库设置
- [SPATIAL_ANALYSIS_MODULE.md](SPATIAL_ANALYSIS_MODULE.md) - 空间分析功能
- [PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md) - 项目结构
- [启动卡死修复.md](启动卡死修复.md) - 启动问题排查

---

**准备好测试了吗？运行 `.\scripts\test_database.bat` 开始！** 🚀

